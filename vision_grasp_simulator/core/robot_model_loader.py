...
class RobotModelLoader:
    ...
    def load(self) -> RobotVisualModel:
        ...
        model.messages.append("已加载 AUBO i10 真实外观模型" if model.using_real_aubo_mesh else "未找到 AUBO i10 mesh，已使用程序化模型替代")
        model.messages.append("已加载 PGC_300_60 电爪模型" if model.using_real_gripper_mesh else "未找到 PGC_300_60 STL，已使用程序化夹爪模型替代")
        return model
    ...
    def _try_load_mesh(self, path: Path):
        try:
            import trimesh
