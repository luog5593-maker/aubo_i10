...
class RobotViewPanel(QWidget):
    ...
    def draw(self,points=None,joint_angles=None):
        ...
        transforms = self._chain_transforms(joint_angles or [0,0,0,0,0,0])
        if self.model:
    ...
    def _chain_transforms(self, angles_deg):
        ...
        if not self.model or not self.model.joints: return transforms
    ...
    def _draw_gripper(self, ax, wrist_T):
        ...
        for i,(name,T) in enumerate(parts):
            info=self.model.gripper_meshes.get(name) if self.model else None; self._draw_part(ax,info,T,10+i)
    ...
    def _draw_part(self, ax, info, T, color_idx):
        ...
        if info and info.mesh_loaded and info.vertices is not None and info.faces is not None:
        ...
        sx,sy,sz=(info.size if info else (.08,.08,.08)); self._draw_box(ax,T,sx,sy,sz,colors[color_idx%len(colors)])
    ...
    def _draw_markers(self,ax):
        if self.target: ax.scatter(self.target[0]/1000,self.target[1]/1000,self.target[2]/1000,c='red',s=60,label='目标点')
        if self.pre: ax.scatter(self.pre[0]/1000,self.pre[1]/1000,self.pre[2]/1000,c='orange',s=50,label='预抓取点')
        if self.path:
            p=np.asarray(self.path)/1000.0; ax.plot(p[:,0],p[:,1],p[:,2],'g--',label='抓取路径')
        if self.target or self.pre or self.path: ax.legend(loc='upper left')
